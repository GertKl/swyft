...
class ZarrStore:
    ...
    def simulate(self, sampler, max_sims=None, batch_size=10, progress_bar=True):
        total_sims = 0
        ...
        while self.sims_required > 0:
            if max_sims is not None and total_sims >= max_sims:
                break
            num_sims = self._simulate_batch(sampler, batch_size,progress_bar=progress_bar)
            total_sims += num_sims

    def _simulate_batch(self, sample_fn, batch_size, progress_bar=True):
        # Run simulator
